...
class HrvFeaturesComplete {
  ...
  static Map<String, double> computeFrequencyDomain(List<double> rrIntervalsMs) {
    ...
    if (resampledRr.length < 16) {
      return {
        'HF': 0.0, 'LF': 0.0, 'VLF': 0.0,
        'HF_nu': 0.0, 'LF_nu': 0.0, 'LFHF': 0.0, 'TP': 0.0,
      };
    }
  ...
  static List<double> _resampleRrIntervals(List<double> rrIntervalsMs, {double samplingRate = 4.0}) {
    ...
    for (int i = 0; i < numSamples; i++) {
      ...
      if (idx == times.length - 1) {
        ...
        final rr1 = idx + 1 < rrIntervalsMs.length ? rrIntervalsMs[idx + 1] : rr0;
  ...
  static List<double> _welchPsd(List<double> signal, {double samplingRate = 4.0}) {
  ...
  static double _bandPower(List<double> psd, double lowFreq, double highFreq, double samplingRate) {
    final n = psd.length;
  ...
  static double computeSd1Sd2(List<double> rrIntervalsMs) {
    ...
    final varDiff = diffs.map((d) => pow(d - meanDiff, 2)).reduce((a, b) => a + b) / diffs.length;
  ...
  static double computeSampleEntropy(List<double> rrIntervalsMs, {int m = 2, double? r}) {
    final validRr = rrIntervalsMs.where((rr) => rr >= 300 && rr <= 2000).toList();
  ...
  static double _dfaFluctuation(List<double> cumSum, int boxSize) {
    ...
    for (int i = 0; i < numBoxes; i++) {
      ...
      final intercept = segment.reduce((a, b) => a + b) / boxSize - slope * (boxSize - 1) / 2;
